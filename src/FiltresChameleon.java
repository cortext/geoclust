...
//		System.out.println("IDp�les = "+IDpbc);
		...
		for(int i=0; i<dbs.MNumClust.size(); i++)
		{
			...
			if(dbs.MNumClust.get(i)!=0)
			{
				...
				}else
				{
					Clusters.add(new Vector<Object>());
					Clusters.get(nb).add(dbs.MNumClust.get(i));
					Clusters.get(nb).add((int) IDpbc.get(i).get(0));//que pour requ�te sur publications pour le moment
					
					num.add(dbs.MNumClust.get(i));
					
					ClustersID.add(new Vector<Object>());
					ClustersID.get(nb).add(dbs.MNumClust.get(i));
					ClustersID.get(nb).add((int) IDpbc.get(i).get(1));//que pour requ�te sur publications pour le moment
...
//		System.out.println("num�ros des clusters = "+num);
//		System.out.println("MNumClust = "+dbs.MNumClust);
		int cluster_visite=1;
		for(int i=0; i<Clusters.size()-1; i++)
		{
//			int nbPubPat1 = 0;
//			for(int k=1; k<Clusters.get(i).size(); k++)//on comptabilise le nb de publications au sein d'un m�me cluster
//			{
		...
		for(int k=1; k<(int)Clusters.get(i).size(); k++)//attention, z�ro est le num�ro du cluster
		{
			...
			for(int l=1; l<(int)Clusters.get(j).size(); l++)//attention, z�ro est le num�ro du p�le
			{
				...
				if(i==j)
				{
					if(k!=l)
					{
						for(int m=1; m<(int)CPubPat.get(a).size(); m++)//attention, z�ro est le num�ro du p�le
						{
							for(int n=1; n<(int)CPubPat.get(b).size(); n++)
							{
								if((int)CPubPat.get(a).get(m) == (int) CPubPat.get(b).get(n))//si les deux p�les ont un m�me id de publication
								{
									nb ++;
								}
							}
				...
				{
					for(int m=1; m<(int)CPubPat.get(a).size(); m++)//attention, z�ro est le num�ro du cluster
					{
						for(int n=1; n<(int)CPubPat.get(b).size(); n++)
						{
							if((int) CPubPat.get(a).get(m) == (int) CPubPat.get(b).get(n))//si les deux p�les ont un m�me id de publication
